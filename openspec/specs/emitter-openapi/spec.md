...
- **WHEN** IR has Parcel with `area: float` and `cadastral_number: string`
- **THEN** `components/schemas/Parcel` SHALL have `area: {type: number, format: float}` and `cadastral_number: {type: string}`

#### Scenario: Request body from permit
- **WHEN** IR permit is `["cadastral_number", "area"]`
